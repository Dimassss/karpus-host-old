 ...
 * License: MIT license
 *
 * Website: http://kencorbettjr.github.io/websql/
 *
...
var websql = (function () {
    ...
    /**
     * sqlProcessor is the core of websql and is responsible for executing all
    ...
    function sqlProcessor(transaction, sqlObjects, finalSuccess, failure) {
        var sequenceNumber = 0,
            ...
            successCallback = function (transaction, results) {
                ...
                if(websql.putSelectResultsInArray && isSelectStmt(sqlObjects[sequenceNumber].sql)){
    ...
    websql.openDatabase =  function (name, displayname, size, whenOpen) {
        websql.database = openDatabase(name, "", displayname, size);
        readTransactionAvailable = typeof websql.database.readTransaction === 'function';
        if (whenOpen) {
            whenOpen();
        }
    }
    ...
    websql.process = function (sqlInput, finalSuccessCallback, failureCallback) {
        if (websql.database) {
            ...
            if(isUndefined(failureCallback)){
                failureCallback = websql.defaultFailureCallback;
            }

            if (allStatementsAreSelectOnly(sqlObjects) && readTransactionAvailable) {
                websql.database.readTransaction(function (transaction) {
                    sqlProcessor(transaction, sqlObjects, finalSuccessCallback, failureCallback);
                }, failureCallback);
            } else {
                websql.database.transaction(function (transaction) {
                    sqlProcessor(transaction, sqlObjects, finalSuccessCallback, failureCallback);
                }, failureCallback);
            }
        } else {
    ...
    /* This is the same as websql.process but used when you want to change the
     ...
     *
     */
    websql.changeVersion = function (oldVersion, newVersion, sqlInput, finalSuccessCallback, failureCallback) {
        if (websql.database) {
            if(websql.database.version === oldVersion){
                var sqlObjects = sqlObjectCreator(sqlInput);
                ...
                websql.database.changeVersion(oldVersion, newVersion, function (transaction) {
                    sqlProcessor(transaction, sqlObjects, finalSuccessCallback, failureCallback);
                }, failureCallback);
            }
        } else {
