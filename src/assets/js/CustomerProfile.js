...
e.bind("table#customers tbody", "load", c.addCustomersToTable);
e.bind("table#orders tbody", "scroll", (e) => {
  let tbl = q("table#orders tbody");
  let dH = tbl.scrollHeight - tbl.scrollTop;
  if(dH < 400) c.addOrdersToTable(e);
});
e.bind("table#orders tbody", "load", c.addOrdersToTable);
...
e.bind('label[for="open-alert-window"]', "click", c.createOrder);
